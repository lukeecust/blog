...
5. **重复 (Repeat)**: 重复步骤 2 和步骤 3 大量次数 (例如 B 次，通常 B 至少为 1000，甚至更多，如10000次，以获得更稳定的结果)，得到 B 个自助统计量 $ \hat{\theta}^*\_1, \hat{\theta}^*\_2, ..., \hat{\theta}^*\_B $。
6. **重复 (Repeat)**: 重复步骤 2 和步骤 3 大量次数 (例如 B 次，通常 B 至少为 1000，甚至更多，如 10000 次，以获得更稳定的结果)，得到 B 个自助统计量  $\hat{\theta}^{*}\_{1}, \hat{\theta}^{*}\_{2}, \ldots, \hat{\theta}^{*}\_{B}$。
7. **统计推断 (Statistical Inference)**: 用这 B 个 Bootstrap 统计量 $ \\{\hat{\theta}^*\_1, ..., \hat{\theta}^*\_B\\} $ 构建一个经验抽样分布。这个分布被用作对真实统计量 $ \hat{\theta} $ 抽样分布的近似。基于这个分布，我们可以：
