...
*   **估计量的标准误差的 Bootstrap 估计**:
    设 $$ \hat{\theta} $$ 是基于原始样本 $$ x_1, ..., x_n $$ 计算得到的参数 $$ \theta $$ 的估计量。我们生成 B 个 Bootstrap 样本，并为每个样本计算相应的 Bootstrap 估计 $$ \hat{\theta}_1^*, \hat{\theta}_2^*, ..., \hat{\theta}_B^* $$。
    标准误的 Bootstrap 估计 $$ \hat{SE}_{boot}(\hat{\theta}) $$ 可以计算为这些 Bootstrap 估计的标准差：
...
*   **Bootstrap 置信区间 (Percentile Method)**:
    ...
    4. 则 $$ (\hat{\theta}_{(k_1)}^*, \hat{\theta}_{(k_2)}^*) $$ 即为 $$ \theta $$ 的 $$ 1-\alpha $$ Bootstrap 百分位置信区间。
