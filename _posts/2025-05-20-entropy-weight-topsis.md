...
2.  **数据预处理（正向化与标准化）**
    ...
    *   **适度指标：**
        ...
        *   若最佳为一个区间 $[a_j, b_j]$，则：
        ...
        \end{equation}
        $$
        处理后，所有 $z'_{ij}$ 的值都落在 $[0,1]$ 区间内，且都是正向化的。若出现 ${z'_{\{ij\}}=0}$ 的情况，在后续计算信息熵时，会涉及到 $\ln(p\_{ij})$。为避免 $p_{ij}=0$ 导致 $\ln(p_{ij})$ 无意义，通常约定当 $p_{ij}=0$ 时，其在熵值计算中的贡献项 $p_{ij} \ln p_{ij} = 0$。
