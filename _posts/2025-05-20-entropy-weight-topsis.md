...
3.  **权重在两处均使用：**
    *   先构建 $V=({w_j} {z'}_{ij})$，然后确定 $V^+, V^-$，之后计算距离时再用 $D_i^{+} =\sqrt{\sum_{j=1}^m w_j (v_{ij} - V_j^{+})^2}$。这种做法相当于权重被应用了两次（一次在 $v_{ij}$ 中，一次在距离公式的 $w_j$ 中），可能会导致权重的过度放大或不合理解释，除非有特殊的理论依据（例如，外部的 $w_j$ 可能是 $w_j$ 的某个函数，如 $w_j^2$）。一般不推荐这种重复加权。
